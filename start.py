...
class SequenceRobot(robot.Robot):
    ...
    def load_seq(self, seq_name=''):
        ...
        if seq_name=='':
            ...
            seq_names = [seq_name+'_sequence.json']
    ...
    def get_sequences(self):
        ...
        """
        seq_list = list(self.seq_list.keys())
        return sorted(list(self.seq_list.keys()), key=str.casefold)
        return seq_list
...
def handle_input(robot, cmd, args=[]):
    ...
    if cmd=='lat':
        ...
        for _ in range(10):
            ...
            try:
                latency.append(server.latency[0])
            except:
                pass
            time.sleep(0.1)
        print("Mean latency: {}".format(np.average(latency)))

    elif cmd == 's' or cmd == 'rand':
        ...
        if seq in robot.seq_list:
            print("\nPlaying sequence: %s"%(args[0]))
            # print(last_cmd, last_args)
            # iterate through all robots
            for bot in robots:
                # don't interrupt sequence if currently playing
                if bot.seq_thread is not None and bot.seq_thread.is_alive():
                    if last_args and len(last_args)>0 and last_args[0]==args[0]:
                        print("Currently playing, not interrupting")
                else:
                    if not bot.seq_stop:
    ...
    # reload gestures
        ...
        print(f"Reloading sequence{'s...' if not args else ': '+args[0]}")
        for bot in robots:
            bot.load_seq('' if not args else args[0].replace(':','/'))
        ...
        for bot in robots:
            if (args[0] == 'all'):
                bot.goto_position({'tower_1': float(args[1]), 'tower_2': float(
                    args[1]), 'tower_3': float(args[1])}, 0.01, True)
            else:
                bot.goto_position({args[0]: float(args[1])}, 0.01, True)
...
def stop_record(robot, seq_name=""):
    ...
    # otherwise, give it random remporary name
    else:
...
def start_server(host, port, hide_browser):
    ...
    try:
        ...
        port = "8000"
        server.start_server(host, port)
        # print("No connection, could not start server!")
        # while True:
            # continue
